...
- 状态：done，Gloria Universe v1.5.1 后台显示名与缓存修复
- 目标：把主题管理卡片、设置按钮和托管设置页标题统一为带空格的 `Gloria Universe`，解决已部署后台仍显示内部短标识的问题。
- 完成：manifest 可见名称改为 `Gloria Universe`；保留服务器文件夹所需的合法内部短标识 `GloriaUniverse`；管理页按钮显示 `Gloria Universe设置`，托管页标题精确显示 `Gloria Universe主题管理`；管理后台入口及相关动态脚本增加 `gu151` 资源标识，避免浏览器沿用 v1.5.0 的同名缓存。
- 验证：ESLint、Vue 类型检查、Vite production build 通过；最终 ZIP 内 manifest、三个精确显示名称、新管理入口和旧入口移除均已核验。
- 输出：`GLORIA-UNIVERSE-Komari-Theme-v1.5.1.zip`，SHA-256 `1252AB4E29A43DF7636F3F8360F3B85AD5EA917E6B059D66CFA254F9F936A724`。

- 状态：done，GLORIA UNIVERSE v1.5.0 后台命名统一与授权歌曲片段试听
- 目标：统一前台、后台与主题侧边栏的 GLORIA 星网命名；为节点歌曲标签和星网地区亮点增加可选的 20–30 秒悬停试听能力。
- 完成：侧边栏显示 `Gloria Universe主题设置`，主题管理标题显示 `Gloria Universe 主题管理`；内部短标识仍保留无空格的 `GloriaUniverse` 以兼容主题路径；原“世界巡演星图 / 巡演星图 / GLORIA WORLD TOUR NETWORK”统一为“GLORIA 星网地图 / GLORIA STAR NETWORK”。
- 音频：主题默认关闭试听且不附带、不抓取任何歌曲；管理员通过后台 JSON 配置自行提供并确认有权使用的本地或 HTTPS 片段。节点分配优先使用用户指定的 20 首歌曲，其余歌曲随机补足；星网地区支持独立歌曲映射；全局单播放器切换并限制 20–30 秒，悬停离开时淡出停止。
- 验证：55 项后台设置键唯一、均有前端对应，下拉项默认值完整；ESLint、Vue 类型检查和 Vite production build 通过；Chrome 完整视觉与交互回归 20/20 通过，授权音频映射聚焦测试 1/1 通过；最终包确认包含试听运行时代码且不含 mp3/wav/m4a/aac/ogg/flac 音频文件。
- 输出：`GLORIA-UNIVERSE-Komari-Theme-v1.5.0.zip`，SHA-256 `95236CC7E0AE7D9C06ADADD23E35486B787E8343CF5140E81BF8947292BE5EC5`。

- 状态：done，GLORIA UNIVERSE v1.3.1 WAKE GLORIA 彩蛋信标
- 目标：为右上角 WAKE GLORIA 增加克制、可发现的彩蛋提示，同时锁定 v1.3.0 使用的婚纱照片，不替换为不同机位或 AI 放大图。
- 完成：菱形使用 2.8 秒呼吸发光；按钮边缘向外扩散柔和金色光波；右上角星芒间歇闪现；悬停反馈与 `prefers-reduced-motion` 保持兼容。
- 素材核验：当前婚纱图为 1440×959；按文件名、画面描述、新闻源和邓紫棋工作室官方婚纱图集检索，未找到可验证为同一张照片的更高分辨率原文件，因此素材未改动。
- 验证：ESLint、`vue-tsc --build`、Vite production build 均通过；Chrome 16/16 完整视觉与交互回归通过。
- 输出：`GLORIA-UNIVERSE-Komari-Theme-v1.3.1.zip`，SHA-256 `99893174CFF3318C986DFCB08C18645D5105525F318FAF52F9F39B01A702A68B`。

- 状态：done，GLORIA UNIVERSE v1.3.0 世界巡演节点星图与婚纱彩蛋
- 目标：首页移除人物照片和旋转地球，改为平铺世界地图；节点国家/地区显示橙金呼吸点，悬停时以绿色点亮对应板块；恢复粉丝主题中文总览；WAKE GLORIA 保持右上角并使用 IAG 2.0 婚纱原图强化“如果神让你看见”。
- 里程碑：M4 UI/UX + M5 粉丝交互；不改变 Komari 数据、权限与路由契约。
- 素材：世界国界来自 Natural Earth 110m 公共领域 GeoJSON，内置于 `public/data/gloria-world-countries.geojson`；婚纱舞台 JPEG 来自中天新闻网公开页面，保留原图并更新 `PHOTO-CREDIT.txt`；v1.2 黑金首页照片已移除。
- 完成：新增 SVG 世界地图投影组件；同一国家的多节点聚合为一个点，支持鼠标和键盘聚焦；港澳节点按中国板块高亮，小型地区仍保留绿色点位反馈；移动端首屏高度压缩；六项文案改为记忆水晶、星辰余辉、能量输出、星库、累计能量、能量接收；彩蛋婚纱背景透明度 0.43，中心句子使用高对比金白发光。
- 验证：ESLint 通过；`vue-tsc --build` 通过；Vite production build 通过；Chrome 16/16 视觉与交互回归通过，覆盖 1280×720、390×844、国家板块悬停高亮、婚纱彩蛋、卡片/列表与详情页；桌面、移动端和彩蛋截图均人工检查。
- 输出：`GLORIA-UNIVERSE-Komari-Theme-v1.3.0.zip`，SHA-256 `16A4084F674B33D8047F6670590385CF4D8F89BFCA9A5D1AFEFA2E75231E21B8`。

- 状态：done，GLORIA UNIVERSE v1.2.0 人物原图、节点地球与总览指标修订
- 目标：统一可见 GLORIA 大写；彩蛋改为“如果神让你看见”；用可追溯的 I AM GLORIA 2.0 黑金权杖舞台原图替换 AI 人物；恢复原模板六项总览；以旋转地球和球面国旗替换地域标签；作者改为 TonyStarkJr2021。
- 素材：人物图来自精彩大马公开新闻页所示 I AM GLORIA 2.0 黑金舞台照片，保留原 JPEG 并附 `PHOTO-CREDIT.txt`；v1.1 AI 人物 WebP 已从源码和新包移除。
- 完成：人物固定在右侧独立区域，所有文字与地球位于左侧；COBE 节点地球自动旋转并按后端地域在球面显示国旗；六项顺序固定为内存、剩余价值、实时上行、硬盘、累计流量、实时下行，并迁移 v1.1/基础旧顺序；中英标签强制单行且完整显示；浏览器标题、设置、页脚和无障碍文案统一 GLORIA 大写；彩蛋句子更新；页脚显示 `Theme GLORIA UNIVERSE · by TonyStarkJr2021`。
- 验证：ESLint 通过；`vue-tsc --build` 通过；Vite production build 通过；Chrome 15/15 视觉与交互回归通过；额外在 1280×720 与 390×844 本地浏览器检查球体、国旗、六项顺序、单行标签、无横向溢出和新彩蛋文案；最终预览图由带模拟节点数据的实际页面生成。
- 输出：`GLORIA-UNIVERSE-Komari-Theme-v1.2.0.zip`，SHA-256 `0CC110FD447D8750CFA2E5F036AA6E3E31129905B456E74901144750280C9EAF`。

- 状态：done，Gloria Universe v1.1 首屏与粉丝体验修订
- 目标：根据实际部署截图压缩首屏留白、完整显示统计中文、保证当前节点批次歌曲标签随机且不重复、用 Gloria 全息人物舞台替换紫晶球、恢复地域信息并实现可发现彩蛋。
- 里程碑：M4 UI/UX + M5 粉丝交互；不修改 Komari 后端、实时数据或权限边界。
- 素材：使用内置 imagegen 生成原创 Gloria 全息舞台人物素材；默认内置，同时支持管理员配置自有图片或静音循环视频。
- 完成：首屏总览由 24rem 固定留白改为内容驱动高度；统计标题允许完整换行；67 首 G.E.M. 曲目按会话洗牌并对当前节点集合唯一分配；紫晶球替换为原创 Gloria 全息人物舞台；舞台显示前五个地域、节点数量及其余地域数量；支持管理员自定义图片或静音循环视频；Crystal G 与 WAKE GLORIA 均可触发全屏星光彩蛋。
- 验证：ESLint 通过；`vue-tsc --build` 通过；Vite production build 通过；Chrome 15/15 视觉与交互回归通过，覆盖 1280×720、390×844、卡片/列表、详情和 Ping；彩蛋 DOM 触发检查通过；最终 ZIP 清单、v1.1.0 manifest、内置人物素材和 `dist/index.html` 均已核验。
- 输出：`Gloria-Universe-Komari-Theme-v1.1.0.zip`，SHA-256 `7E96BBF70EE1CB21A7DB3423C23340E0BE07D5247AE0EC2DD331C95B848394E5`。

- 状态：done，Gloria Universe v1.0 已完成并通过构建与视觉验证
- 目标：依据 Gloria Universe Theme Design v1.0 与 v1.1，将 Glassmorphism v3 数据与打包架构改造为 G.E.M. 粉丝向的宇宙舞台监控主题。
- 里程碑：M4 UI/UX；保留 Komari API、实时指标、权限和公开路由行为。
- 范围：原创舞台深空背景、Crystal G、Gloria Core、紫蓝粉金色板、品牌头部、总览卡片、节点星辰卡片、移动端与低动态适配、预览与主题包。
- 不做：不打包官方照片、音乐、演唱会视频或官方 Logo；不发布到外部平台；不修改 Komari 后端。
- 实现：新增 Crystal G、Gloria Core、舞台深空背景、紫蓝粉金色板、品牌头部、宇宙总览、星辰节点语言、地区歌曲彩蛋和两列手机总览；移除首页 WebGL 地球依赖路径。
- 验证：ESLint（含格式修复）通过；`vue-tsc --build` 通过；Vite production build 通过；1280×720 与 390×844 实际浏览器检查无横向溢出；首页 → 详情 → 首页往返正常；生成 v1.0.0 Komari ZIP。
- 素材：内置 imagegen 生成原创舞台深空背景与透明 Gloria Core，分别压缩为约 106 KB 与 318 KB WebP；Crystal G 为项目 SVG。
...
## 2026-08-25 GLORIA UNIVERSE v1.4.0 identity and starlight theme

- Replaced the generic visitor identities with fan-facing GLORIA titles: unauthenticated visitors are `GLORIA 棋士`, while authenticated administrators are `GLORIA 领航员`.
- Reworded the visitor status into the theme narrative: successful lookup reports that the location's starlight has joined the GLORIA network, with matching loading and fallback states.
- Rebuilt starlight mode as a pearl-white and crystal-lavender stage palette with deep-purple text, cyan/purple/rose/gold metric accents, translucent lavender cards and stronger readable borders. Deep-space mode remains unchanged.
- Added deterministic guest and authenticated visual coverage for the new identities. The complete Chrome visual suite passed 18/18 after baseline review; ESLint, Vue type-checking and the Vite production build also passed.
- Final deliverable: `GLORIA-UNIVERSE-Komari-Theme-v1.4.0.zip`, SHA-256 `2DE99E4FFF13DBAC4BFDA62B9AE515F369AF37C7E0FDC03C637E3689DB4AB8D0`. Archive verification confirmed manifest version `1.4.0`, short name `GloriaUniverse`, `preview.png`, and `dist/index.html`.

## 2026-08-25 GLORIA UNIVERSE v1.4.1 starlight redesign

- Identity labels now display only `棋士` for guests and `领航员` for authenticated administrators, without a `GLORIA` prefix.
- Starlight mode now has its own generated high-key iridescent crystal-stage background (`/images/starlight-background-v1.png`). Deep-space mode continues to use `/images/default-background-v2.webp`, so the two modes no longer share one dark concert image.
- Light-mode cards use more transparent layered cyan/violet/pink glass, stronger pearl borders and deep-purple foreground/muted text. The GLORIA world map now has a bright pearl surface and matching readable light-mode map labels.
- Fixed the visitor-card dark selector scope and restored the deep-space visitor detail card to understated neutral black glass; the light-mode visitor card retains the iridescent treatment.
- Validation: ESLint, Vue type-checking and the production build passed. The full Chrome visual suite passed 19/19, including new starlight guest and deep-space guest desktop baselines; a targeted post-fix deep-space visual check also passed.
- Final deliverable: `GLORIA-UNIVERSE-Komari-Theme-v1.4.1.zip`, SHA-256 `20BA5148131229F6C4A21335A6B24C46F85B6BDB08A6C88E2EE277618B4700CD`. Archive verification confirmed manifest version `1.4.1`, `preview.png`, `dist/index.html`, and `dist/images/starlight-background-v1.png`.

## 2026-08-25 GLORIA UNIVERSE v1.4.2 managed-settings copy audit

- Audited all 50 managed theme settings against their frontend consumers. Every manifest key is referenced by the runtime, there are no duplicate keys, and every select default remains present in its options.
- Replaced stale generic light/dark wording with the frontend names `星光模式` / `深空模式`; settings sections now follow the GLORIA narrative for the tour map, WAKE GLORIA easter egg, fan tracks, starlight nodes and navigator-only tools.
- The guest/admin setting now documents the exact frontend identities `棋士` / `领航员`. The six default overview keys document the exact visible labels: `CRYSTAL MEMORY · 记忆水晶`, `STAR VALUE · 星辰余辉`, `ENERGY OUT · 能量输出`, `STAR VAULT · 星库`, `ENERGY ARCHIVE · 累计能量`, and `ENERGY IN · 能量接收`.
...
- Release follow-up: the first GitHub Linux Chromium run found a 2 px overflow for the `1009 KB/s` fixture that did not reproduce in local Edge. Reduced only the icon/value gap in the two live-speed rows by 2 px, preserving the three-column layout while adding cross-platform font-width tolerance.

## 2026-08-28 GLORIA UNIVERSE v1.5.5 local Crystal G easter-egg candidate

- User selected the first researched concert-light photo for a new left-header Crystal G easter egg and explicitly requested a local test package only; no commit, push, tag, Release, or GitHub upload is authorized.
- Split the previously shared easter event into independent variants: the Crystal G trigger uses the 1500x999 I AM GLORIA radial crystal-stage photo and “见过光，就应该把光反射出去”, while the map-side WAKE GLORIA trigger retains the wedding-dress photo, custom-media settings, and “如果神让你看见”.
- Kept the existing easter content structure, palette, animation, timing, and layout. Added explicit trigger markers and a visual regression for the independent Crystal G flow.
...
- Validation passed: ESLint, Vue type-checking, production Vite build, `git diff --check`, archive inspection, and the complete Edge visual suite (`26/26`). Dedicated 1280x720 and 390x844 baselines confirm that the new sentence remains readable without horizontal overflow; the existing WAKE GLORIA wedding flow also passed unchanged.
- Local-only deliverable: `GLORIA-UNIVERSE-Komari-Theme-v1.5.5-local-test.zip`, SHA-256 `BD816D09F2445CD8AD2D6F002693C4C06617C0B50093D69DECD4CC9F9884015A`. The archive contains manifest version `1.5.5`, configuration label `主题设置`, `preview.png`, `dist/index.html`, and the new reflected-light image. Nothing was uploaded to GitHub.

## 2026-08-28 GLORIA UNIVERSE v1.5.6 local endurance-inscription candidate

- Added the exact short phrase `惟有忍耐到底的` as a dedicated vertical stage inscription in the unused wide-screen gutter. After two real-browser zoom tests showed that viewport-proportional placement could still enter the dashboard, the inscription now uses a fixed 3rem left inset and appears only from 1600px by 900px. Its visual regression asserts that its right edge remains left of the first statistic card and that its bottom remains above the visitor card. The existing background image is unchanged.
- Replaced the framed `光` diamond below the sentence with a compact iridescent `……` continuation mark, as requested.
- Root cause from the next real-browser test: the home entrance animation's retained `transform` made a nested fixed element use the centered home component as its containing block. The inscription is now teleported directly to `body`, so its fixed 3rem inset is genuinely viewport-relative regardless of the dashboard transform, browser zoom, or content centering. It is also gated by the HomeView activation state so it disappears on detail routes. The ellipsis is now three vertically stacked cyan/gold/lilac glowing dots.
- The inscription uses a distinct Chinese calligraphic serif stack, champagne-gold-to-lilac lettering, a small `光` diamond seal, a restrained vertical English guide, and a slow reduced-motion-aware glow. It is hidden below 1440px or 700px high so tablet/mobile layouts and the main 1280px dashboard never lose space or readability.
- Validation passed: ESLint, Vue type-checking, production Vite build, `git diff --check`, archive inspection, and the complete Edge visual suite (`27/27`). A dedicated 1920x1080 baseline confirms that the inscription remains entirely inside the left gutter and does not overlap the dashboard.
- Local-only deliverable after the root-level fixed overlay and vertical-dot correction: `GLORIA-UNIVERSE-Komari-Theme-v1.5.6-local-test.zip`, SHA-256 `DC52C6713A1F848F4B8621D53E3FE8BD64C5FA9DD25AA0B96B995C8D743A5419`. Embedded manifest version is `1.5.6`; no commit, push, tag, Release, or GitHub upload was performed.

## 2026-08-28 GLORIA UNIVERSE v1.5.5 formal release preparation

- The user completed local testing and explicitly requested this combined update be published as `v1.5.5` (not the temporary local candidate number `1.5.6`). Commit, push, automated tag/Release creation, release notes, Full Changelog verification, and asset verification are authorized.
- Release scope: independent Crystal G reflected-light easter egg and sentence; unchanged WAKE GLORIA wedding easter egg; viewport-root wide-screen inscription `惟有忍耐到底的` with three vertical glow dots; dedicated desktop/mobile/collision regressions; material credit update.
- Refreshed both `preview.png` and `docs/preview.png` from the same unobstructed 3200x1800 deep-space fixture with visitor information disabled. SHA-256 for both preview files is `DB882E63ABE045262D06668F92DD8F6EC0A10C32A67552B47A41C2CC7AB3FB56`.
- Local validation before push: ESLint, Vue type-checking, production build, `git diff --check`, the focused 3200x1800 preview regeneration, and the full local Edge visual suite (`27/27`) passed.
